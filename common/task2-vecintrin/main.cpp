...
int main(int argc, char * argv[]) {
  ...
  if (printLog) ECE521Logger.printLog();
  ECE521Logger.printStats();
...
// implementation of absSerial() above, but it is vectorized using ece521 intrinsics
void absVector(float* values, float* output, int N) {
  __ece521_vec_float x;
  __ece521_vec_float result;
  __ece521_vec_float zero = _ece521_vset_float(0.f);
  __ece521_mask maskAll, maskIsNegative, maskIsNotNegative;
...
//  Why is that the case?
  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    ...
    // All ones
    maskAll = _ece521_init_ones();

    // All zeros
    maskIsNegative = _ece521_init_ones(0);

    // Load vector of values from contiguous memory addresses
    _ece521_vload_float(x, values+i, maskAll);               // x = values[i];

    // Set mask according to predicate
    _ece521_vlt_float(maskIsNegative, x, zero, maskAll);     // if (x < 0) {

    // Execute instruction using mask ("if" clause)
    _ece521_vsub_float(result, zero, x, maskIsNegative);      //   output[i] = -x;

    // Inverse maskIsNegative to generate "else" mask
    maskIsNotNegative = _ece521_mask_not(maskIsNegative);     // } else {

    // Execute instruction ("else" clause)
    _ece521_vload_float(result, values+i, maskIsNotNegative); //   output[i] = x; }

    // Write results back to memory
    _ece521_vstore_float(output+i, result, maskAll);
  }
}
...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  //
  // ece521 STUDENTS TODO: Implement your vectorized version of
...
float arraySumVector(float* values, int N) {
  ...
  //
  // ece521 STUDENTS TODO: Implement your vectorized version of arraySumSerial here
  //
